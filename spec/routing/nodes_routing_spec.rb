  ...
  before(:each) do
   ...
   describe "routing" do
     ...
     it "recognizes and generates #reorder" do
       expected_path = '/test-user/piles/1/nodes/2/reorder'
       
       { :put => expected_path }.should route_to(:controller => 'nodes', :action => 'reorder', :user_id => 'test-user', :pile_id => '1', :id => '2')
       reorder_user_pile_node_path(:id => 2, :pile_id => 1, :user_id => 'test-user').should == expected_path
       polymorphic_path([:reorder, @user, @pile, @node]).should == expected_path
     end
     
     it "recognizes and generates #reparent" do
       expected_path = '/test-user/piles/1/nodes/2/reparent'
       
       { :put => expected_path }.should route_to(:controller => 'nodes', :action => 'reparent', :user_id => 'test-user', :pile_id => '1', :id => '2')
       reparent_user_pile_node_path(:id => 2, :pile_id => 1, :user_id => 'test-user').should == expected_path
