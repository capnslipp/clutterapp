class String
  def self.rand_alphanum(length = 20)
    chars = ('a'..'z').to_a + ('A'..'Z').to_a + ("0".."9").to_a
    hash = ""
    length.times { hash << chars[ rand(chars.size) ] }
    hash
  end
  
  def self.rand_lorem(word_count = 20)
...
class Node < ActiveRecord::Base
  ...
  #validates_presence_of :pile, :if => :root?
  ...
  def self.create_rand
    prop = [TextProp, CheckProp, NoteProp, PriorityProp, TagProp, TimeProp].rand.new
    case prop
      when TextProp:      prop.text = String.rand_lorem(5)
      when CheckProp:     prop.checked = (rand(1 + 1) == 1)
      when NoteProp:      prop.note = String.rand_lorem(50)
      when PriorityProp:  prop.priority = rand(5 + 1)
      when TagProp:	  prop.tag = String.rand_alphanum(4)
      when TimeProp:	  prop.time = Time.now - rand(59).minutes - rand(23).hours - rand(364).days - rand(100).years
      else
        return nil
    end
    self.create :prop => prop
  end
  
  def self.create_rand_in_pile(pile)
    node = self.create_rand
    parent_node = (pile.root_node.descendants << pile.root_node).rand
